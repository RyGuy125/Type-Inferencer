...
fun solve TRIVIAL       = idsubst
  | solve (tau1 ~ tau2) = 
      (case (tau1, tau2) 
        of (TYVAR a1, TYVAR a2)  => a1 |--> TYVAR a2
         | (TYVAR a1, TYCON mu2) => a1 |--> TYCON mu2
         | (TYVAR a1, CONAPP (t2,ts2)) => 
             if List.exists 
                  (fn (t) => eqType (TYVAR a1, TYVAR t))
                  (freetyvars (CONAPP (t2, ts2)))
             then unsatisfiableEquality (TYVAR a1, CONAPP (t2,ts2))
             else 
                a1 |--> CONAPP (t2,ts2)
         | (TYCON mu1, TYVAR a2) => a2 |--> TYCON mu1
         | (TYCON mu1, TYCON mu2) => 
             if eqType (TYCON mu1, TYCON mu2) then idsubst
             else 
                unsatisfiableEquality (TYCON mu1, TYCON mu2)
         | (TYCON mu1, CONAPP (t2,ts2)) => 
             unsatisfiableEquality (TYCON mu1, CONAPP (t2,ts2))
         | (CONAPP (t1,ts1), TYVAR a2) => solve (TYVAR a2 ~ CONAPP (t1,ts1))
         | (CONAPP (t1,ts1), TYCON mu2) =>
             unsatisfiableEquality (CONAPP (t1,ts1), TYCON mu2)
         | (CONAPP (t1,ts1), CONAPP(t2,ts2)) => 
            solve (conjoinConstraints (ListPair.foldlEq 
                                        (fn (ta1,ta2,acc) => (ta1 ~ ta2) :: acc)
                                        [t1 ~ t2]
                                        (ts1, ts2)
                                        handle _ => unsatisfiableEquality
                                            (CONAPP (t1,ts1), 
                                            CONAPP (t2,ts2)))))
  | solve (c1 /\ c2) =
      let val subst1 = solve c1
          val con = consubst subst1 c2
          val subst2 = solve con
      in 
        compose (subst2, subst1) 
      end
...
fun solutionEquivalentTo (c, theta) = eqsubst (solve c, theta)
        ...
        (* TYVAR to TYVAR *)
        val () = Unit.checkAssert "'a ~ 'a is solved by identity substitution"
                 (fn () => solutionEquivalentTo (TYVAR "'a" ~ TYVAR "'a",
                                                 idsubst)) 
        (* TYVAR to CONAPP with no solution *)
        val () = Unit.checkAssert "'a ~ 'a list has no solution" 
                 (fn () => hasNoSolution (TYVAR "'a" ~ listtype (TYVAR "'a")))

        (* TYVAR to CONAPP with solution *)
        val () = Unit.checkAssert "'a ~ 'b list solved by 'a |--> 'b list"
                 (fn () => solutionEquivalentTo 
                            (TYVAR "'a" ~ listtype (TYVAR"'b"), 
                            "'a" |--> listtype (TYVAR "'b")))
        (* TYCON to TYVAR *)
        val () = Unit.checkAssert "int ~ 'a is solved by 'a |--> int"
                 (fn () => solutionEquivalentTo (inttype ~ TYVAR "'a", 
                                                "'a" |--> inttype))

        (* TYCON to CONAPP *)
        val () = Unit.checkAssert "int ~ int list cannot be solved"
                 (fn () => hasNoSolution (inttype ~ listtype inttype))

        (* CONAPP to TYVAR with no solution *)
        val () = Unit.checkAssert "'a list ~ 'a has no solution"
                 (fn () => hasNoSolution (listtype (TYVAR "'a") ~ TYVAR "'a"))

        (* CONAPP to TYVAR with solution *)
        val () = Unit.checkAssert "int list ~ 'a solved by 'a |--> int list"
                 (fn () => solutionEquivalentTo (listtype inttype ~ TYVAR "'a",
                                                 "'a" |--> listtype inttype))
        ...
        (* CONAPP to CONAPP *)
        val () = Unit.checkAssert "listtype int ~ listtype bool cannot be solved"
                 (fn () => hasNoSolution (listtype inttype ~ listtype booltype))

        (* TYVAR to TYVAR /\ TYVAR to TYCON *)
        val () = Unit.checkAssert "'a ~ 'b AND 'b ~ bool is solved by 'a |--> bool"
                 (fn () => solutionEquivalentTo (alpha ~ beta /\ beta ~ booltype,
                                                 compose ("a" |--> booltype,
                                                          "b" |--> booltype))) 
        (* CONAPP to CONAPP /\ TYVAR to TYCON *)
        val () = Unit.checkAssert ("'a list ~ 'b list AND 'b ~ bool is solved by"
                                  ^ "o 'a |--> bool 'b |--> bool")
                 (fn () => solutionEquivalentTo 
                            (listtype (TYVAR "'a") ~ listtype (TYVAR "'b") 
                                /\ TYVAR "'b" ~ booltype,
                            compose ("'a" |--> booltype, "'b" |--> booltype))) 
        (* TRIVIAL *)
        val () = Unit.checkAssert "TRIVIAL satisfiable"
                 (fn () => hasSolution TRIVIAL)

        (* constraints.sml *)
        val () = Unit.checkAssert "constraints.sml first failure"
                 (fn () => hasNoSolution (   TYVAR "a" ~ TYVAR "b"
                                          /\ TYVAR "b" ~ TYVAR "c"
                                          /\ TYVAR "a" ~ TYCON "int"
                                          /\ TYVAR "c" ~ TYCON "bool"))
        
        val () = Unit.checkAssert "constraints.sml second failure"
                 (fn () => hasNoSolution
        ...
        val () = Unit.checkAssert "constraints.sml success"
                 (fn () => hasNoSolution (
                    CONAPP (TYCON "function", 
...
(* function [[literal]], to infer the type of a literal constant ((prototype)) 448e *)
      fun literal (NUM   n) = (inttype, TRIVIAL)
        | literal (BOOLV b) = (booltype, TRIVIAL)
        | literal (SYM   s) = (symtype, TRIVIAL)
        | literal (NIL )    = (listtype alpha, TRIVIAL)
(*        | literal (PAIR (v, NIL)) =
            let
                val (ty_v, con) = literal v
             in
                (listtype ty_v, TRIVIAL)
            end*)
        | literal (PAIR (v1, v2)) = 
            (case (literal v1, literal v2)
               of ((t1, c1), (t2, c2)) =>
                  (listtype t1, c1 /\ c2 /\ listtype t1 ~ t2)
                (*| _ => raise TypeError "literal-pair secondary failure"*)
                )

        | literal _           = raise TypeError "fn literal list"
(*
fun listtype ty = 
  CONAPP (TYCON "list", [ty])*)
      ...
      fun ty (LITERAL n) = literal n
        ...
        | ty (IFX (e1, e2, e3))        =
              (case typesof ([e1, e2, e3], Gamma)
                 of (tau1 :: tau2 :: [tau3], constraints) =>
                    (tau2, constraints /\ tau1 ~ booltype /\ tau2 ~ tau3)
                  | _ => raise TypeError "IFX failed to infer e1 e2 e3")
        ...
        | ty (BEGIN es)                = 
              (case es
                 of []        => (unittype, TRIVIAL)
                  | (ex :: exs) => 
                      let
                          val (typs, cons) = typesof (es, Gamma)
                       in
                          case List.rev typs
                            of (t :: ts) => (t, cons)
                             | _         => raise TypeError "BEGIN failure"
                      end)
        | ty (LAMBDA (formals, body))  = 
            let 
                val alphas = foldl (fn (a, acc) => freshtyvar () :: acc)
                                   []
                                   formals
                val Gamma' =
                    ListPair.foldlEq (fn (x, a, gam) =>
                                        bindtyscheme (x, FORALL ([], a), gam))
                                     Gamma
                                     (formals, alphas)
                val (typ, con) = typeof (body, Gamma')
             in (funtype (alphas, typ), con)
            end
        ...
        | ty (LETX (LET, bs, body))    = 
              let
                  ...
                  val (etyps, cons) = typesof (es, Gamma)
                  val theta         = solve cons
                  val alphas        = inter (dom theta, freetyvarsGamma Gamma)
                  val cons_list     = map (fn (a) => TYVAR a ~ (varsubst theta a)) alphas
                  val cons'         = conjoinConstraints cons_list
                  val free_ty_vars  = union (freetyvarsGamma Gamma, freetyvarsConstraint cons') (* name set *)
                  val sigma_list    = map (fn (t) => generalize (tysubst theta t, free_ty_vars)) etyps
                  val gamma'        = ListPair.foldlEq (fn (x, sigma, gam) => bindtyscheme (x, sigma, gam)) Gamma (xs, sigma_list)
                  val (e_ty, c_b)   = typeof (body, gamma')
              in (e_ty, cons' /\ c_b)
              end
        ...
        | ty (LETX (LETREC, bs, body)) =
              let
                  val (xs, es)            = ListPair.unzip bs
                  fun is_lambda []        = true
                    | is_lambda (y :: ys) = (case y
                                                of LAMBDA z => is_lambda ys
                                                 | _        => raise TypeError "LETREC failure")
                  val all_lambdas = is_lambda es
                  val alphas = foldl (fn (x, acc) => freshtyvar () :: acc)
                                     []
                                     xs
                  val gamma' =
                      ListPair.foldlEq (fn (x, a, gam) =>
                                       ...
                                       Gamma
                                       (xs, alphas)
                  val (e_typs, c_r) = typesof (es, gamma')
                  (*val cons_list     = ListPair.mapEq (fn (t, a) => t ~ TYVAR a) (e_typs, alphas)*)
                  val cons_list     = ListPair.mapEq (fn (t, a) => t ~ a) (e_typs, alphas)
                  val c             = conjoinConstraints (c_r :: cons_list)
                  val theta         = solve c
                  val alphas'       = inter (dom theta, freetyvarsGamma Gamma)
                  val cons_list'    = map (fn (a) => TYVAR a ~ (varsubst theta a)) alphas'
                  val c'            = conjoinConstraints cons_list'
                  
                  val free_ty_vars  = union (freetyvarsGamma Gamma, freetyvarsConstraint c') (* name set *)
                  val sigma_list    = map (fn (t) => generalize (tysubst theta t, free_ty_vars)) e_typs

                  val gamma''        = ListPair.foldlEq (fn (x, sigma, gam) => bindtyscheme (x, sigma, gam)) Gamma (xs, sigma_list)
                  val (body_ty, c_b) = typeof (body, gamma'')
              in (body_ty, c' /\ c_b)
              end
                         ...
                         ((* primitives for \nml\ and \uml\ [[::]] 451b *)
                          ...
                          ("cdr",   unaryOp
                                    ...
                                    funtype ([listtype alpha], listtype alpha))
                                                                              ::
                          ("pair",  binaryOp (fn (a, b) => embedList (SYM "PAIR" :: [a, b])),
                                    funtype ([alpha, beta],
                                             pairtype (alpha, beta))) ::
                          ("fst", unaryOp (fn NIL => raise RuntimeError "fst applied to NIL"
                                            | PAIR (fst, _) => fst
                                            | _ => raise BugInTypeInference "fst applied to non-pair"),
                                  funtype ([pairtype (alpha, beta)], alpha)) ::
                          ("snd", unaryOp (fn NIL => raise RuntimeError "snd applied to NIL"
                                            | PAIR (_, snd) => snd
                                            | _ => raise BugInTypeInference "snd applied to non-pair"),
